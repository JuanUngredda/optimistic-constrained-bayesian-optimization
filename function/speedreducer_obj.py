...
from juan_benchmark.speedreducer_math import objective_raw


class SpeedReducer_OBJ(BlackBoxFunction):
    def __init__(self, xsize=10, zsize=10, task_identifier="original_1_1", noise_std=0.01):
        xdim = 6
        zdim = 1
        super(SpeedReducer_OBJ, self).__init__(
            xdim, zdim, xsize, zsize, task_identifier, noise_std=noise_std
        )
        self.module_name = "speedreducer_obj"
        self.xsize = xsize
        self.zsize = zsize
    ...
    def get_params(self, task_identifier):
        # Near-delta z-distribution centred at 0.5 (-> physical x7 ~ 5.25).
        # SpeedReducer is deterministic; this collapses regret to the 6D slice.
        return {
