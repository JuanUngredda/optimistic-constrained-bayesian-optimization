...
class Cifar10CNN_CST10(BlackBoxFunction):
    def __init__(
        self,
        xsize=10,
        zsize=10,
        ...
        super(Cifar10CNN_CST10, self).__init__(
            xdim, zdim, xsize, zsize, task_identifier, noise_std=noise_std
        )

        self.module_name = "cifar10_cnn_cst10"
        func_data = Cifar10CNN_CST10.get_objfunc_and_constraints()

        self.xz_domain = torch.from_numpy(func_data["normalized_input"]).float()
        self.vals = torch.from_numpy(func_data["10_constraints"][9]).squeeze().float()
