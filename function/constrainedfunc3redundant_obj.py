...
from juan_benchmark.constrainedfunc3redundant_math import objective_raw


class ConstrainedFunc3Redundant_OBJ(BlackBoxFunction):
    def __init__(self, xsize=10, zsize=10, task_identifier="original_1_1", noise_std=0.01):
        xdim = 1
        zdim = 1
        super(ConstrainedFunc3Redundant_OBJ, self).__init__(
            xdim, zdim, xsize, zsize, task_identifier, noise_std=noise_std
        )
        self.module_name = "constrainedfunc3redundant_obj"
        self.xsize = xsize
        self.zsize = zsize
    ...
    def get_params(self, task_identifier):
        # Near-delta z-distribution centred at 0.5 (-> x1 ~ 0.5).
        # ConstrainedFunc3Redundant is deterministic in x1; this collapses
        # regret to the 1D slice.
        return {
    ...
    def get_noiseless_notransformed_observation(self, xz):
        with torch.no_grad():
            ...
            val = objective_raw(xz).reshape(-1)
        return val.float()
