...
from juan_benchmark.pressurevessel_math import bilog, constraint_raw


class PressureVessel_CST4(BlackBoxFunction):
    def __init__(self, xsize=10, zsize=10, task_identifier="original_1_1", noise_std=0.01):
        xdim = 3
        zdim = 1
        super(PressureVessel_CST4, self).__init__(
            xdim, zdim, xsize, zsize, task_identifier, noise_std=noise_std
        )
        self.module_name = "pressurevessel_cst4"
        self.xsize = xsize
        self.zsize = zsize
    ...
    def get_noiseless_notransformed_observation(self, xz):
        with torch.no_grad():
            ...
            val = bilog(constraint_raw(xz, 4)).reshape(-1)
        return val.float()
