...
class WeldedBeamDesign_2_3_5_CST3(BlackBoxFunction):
    ...
    def __init__(
        ...
        super(WeldedBeamDesign_2_3_5_CST3, self).__init__(
            xdim, zdim, xsize, zsize, task_identifier, noise_std=noise_std
        )

        self.module_name = "welded_beam_design_2_3_5_cst3"
        self.xsize = xsize
        self.zsize = zsize
        ...
        with torch.no_grad():
            ...
            Pc = (
                ...
                * torch.pow(xz[:, 3], 3.0)
                / 6.0
                / L**2
                * (1.0 - xz[:, 2] / 2.0 / L * np.sqrt(E / 4.0 / G))
            )
...
#
            ...
            print(f"Threshold <= {threshold}")  # 0.9877783175228785
