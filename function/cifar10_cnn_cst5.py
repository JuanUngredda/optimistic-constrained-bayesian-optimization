...
class Cifar10CNN_CST5(BlackBoxFunction):
    def __init__(
        self,
        xsize=10,
        zsize=10,
        ...
        super(Cifar10CNN_CST5, self).__init__(
            xdim, zdim, xsize, zsize, task_identifier, noise_std=noise_std
        )

        self.module_name = "cifar10_cnn_cst5"
        func_data = Cifar10CNN_CST5.get_objfunc_and_constraints()

        self.xz_domain = torch.from_numpy(func_data["normalized_input"]).float()
        self.vals = torch.from_numpy(func_data["10_constraints"][int(self.module_name[-1])-1]).squeeze().float()
