...
class WeldedBeamDesign_2_3_5_CST4(BlackBoxFunction):
    ...
    def __init__(
        ...
        super(WeldedBeamDesign_2_3_5_CST4, self).__init__(
            xdim, zdim, xsize, zsize, task_identifier, noise_std=noise_std
        )

        self.module_name = "welded_beam_design_2_3_5_cst4"
        self.xsize = xsize
        self.zsize = zsize
    ...
    def get_noiseless_notransformed_observation(self, xz):
        ...
        with torch.no_grad():
            ...
            delta = 6 * P * L**3 / E / torch.pow(xz[:, 2], 3.0) / xz[:, 3]

            R = torch.sqrt(
                torch.square(xz[:, 1]) / 4.0 + torch.square(xz[:, 0] + xz[:, 2]) / 4
            )

            M = P * (xz[:, 1] / 2.0 + L)
            tau1 = P / np.sqrt(2) / xz[:, 1] / xz[:, 0]
            tau2 = R * M
            ...
            print(f"Threshold <= {threshold}")  # 1.2476827532236885
