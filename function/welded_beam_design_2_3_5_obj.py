...
class WeldedBeamDesign_2_3_5_OBJ(BlackBoxFunction):
    ...
    def __init__(
        ...
        super(WeldedBeamDesign_2_3_5_OBJ, self).__init__(
            xdim, zdim, xsize, zsize, task_identifier, noise_std=noise_std
        )

        self.module_name = "welded_beam_design_2_3_5_obj"
        self.xsize = xsize
        self.zsize = zsize
    ...
    def get_noiseless_notransformed_observation(self, xz):
        ...
        with torch.no_grad():
            ...
            val = (
                0.04811 * xz[:, 2] * xz[:, 3] * (xz[:, 1] + 14.0)
                + 1.10471 * torch.square(xz[:, 0]) * xz[:, 1]
            )
