...
class Cifar10CNN_OBJ(BlackBoxFunction):
    def __init__(
        self,
        xsize=10,
        zsize=10,
        ...
        super(Cifar10CNN_OBJ, self).__init__(
            xdim, zdim, xsize, zsize, task_identifier, noise_std=noise_std
        )

        self.module_name = "cifar10_cnn_obj"
        func_data = Cifar10CNN_OBJ.get_objfunc_and_constraints()

        self.xz_domain = torch.from_numpy(func_data["normalized_input"]).float()
        self.vals = torch.from_numpy(func_data["objective_function"]).squeeze().float()
