...
class ConstrainedFunc3Redundant_CST1(BlackBoxFunction):
    def __init__(self, xsize=10, zsize=10, task_identifier="original_1_1", noise_std=0.01):
        xdim = 1
        zdim = 1
        super(ConstrainedFunc3Redundant_CST1, self).__init__(
            xdim, zdim, xsize, zsize, task_identifier, noise_std=noise_std
        )
        self.module_name = "constrainedfunc3redundant_cst1"
        self.xsize = xsize
        self.zsize = zsize
    ...
    def get_noiseless_notransformed_observation(self, xz):
        with torch.no_grad():
            ...
            val = constraint_raw(xz, 1).reshape(-1)
        return val.float()
