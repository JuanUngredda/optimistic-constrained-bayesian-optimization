...
Classic constrained Bayesian optimization using expected improvement
multiplied by constraint satisfaction probabilities.
"""
import torch

from utils import expected_improvement, gaussian_cdf


class EIC:
    """
    Expected Improvement with Constraints acquisition function.
    
    Computes EI for the objective multiplied by the probability that
    all constraints are satisfied. Always queries both target and
    constraints together (coupled approach).
    ...
    adaptive : bool
        Always False for EIC (coupled queries only).
    
    Notes
    -----
    EIC(x) = EI(x) × ∏ P(g_i(x) ≥ threshold_i)
    
    Always queries 2 evaluations per iteration: target AND constraint.
    """
    def __init__(self):
        self.estimator_idx = None
        self.estimator = None
        # for EIC use input query as the estimator (or the best observation so far?)
        self.adaptive = False  # only coupled queries for EIC
    ...
    def expected_improvement_with_constraints(
        ...
        constraint_mean_f_list,  # already transformed so that inequality is >=
        constraint_std_f_list,
        threshold_list,  # already transformed so that inequality is >=
    ):
    ...
    def get_least_violated_input(
        ...
        # maximum target function value
        # such that all constraint are satisfied
    ...
    def get_best_feasible_observed_input(
        ...
        if len(feasible_idxs):
            ...
            #                 threshold = threshold_list[cidx]
            #                 if constraint_info["inequality"] == "less_than_equal_to":
            #                     print(cidx, moidx, -constraint_y[idx], threshold, -constraint_y[idx] >= threshold)
            #                 else:
            #                     print(cidx, moidx, constraint_y[idx], threshold, constraint_y[idx] >= threshold)
            #
            #             print("=======")
            ...
            return idx
        return None  # NOTE: return None if cannot find any feasible inputs
    ...
    def get_query(
        ...
        other_info,  # {'constraint_info_list', 'target_xz', 'target_y', 'cidx_to_moidx', 'constraint_y_dict'}
    ):
        ...
        if best_observed_target_y_idx is None:
            ...
            #             best_observed_target_y_idx = EIC.get_least_violated_input(
            #                 target_y,
            #                 constraint_y_dict,
            #                 constraint_info_list,
            #                 cidx_to_moidx,
